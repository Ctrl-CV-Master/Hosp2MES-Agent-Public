"""Browser-mode agent: real GUI execution of MES business workflows.

This is the browser counterpart of :class:`hosp2mes.agent.agent.Agent`. It reuses
the *same* Planner, Progress Memory, Evidence Verifier and Evaluator, but instead
of calling the MES REST API it drives the Vue Mock MES through
:class:`~hosp2mes.observation.browser_env.BrowserEnv` and
:class:`~hosp2mes.executor.browser_executor.BrowserExecutor`.

Design invariants:

* **Observe first.** Before every action the agent re-reads the rendered page and
  records a structured observation + screenshot.
* **Semantic actions only.** Each subgoal is decomposed into abstract GUI actions
  (click / type / select / ...) that reference controls by accessible name, not
